...
export default {
  ...
  data() {
    return {
      boxx: {}, // 展示的 title 和 content
      class_boxx: "", // 展示的 class 样式
      list: this.getRandomList(10), // 默认使用随机的 10 条数据
      type_boxx_list: type_boxx_list, // type 样式数组
      show_content: true,
      show_title: true,
      style_boxx: {},
      style_title: {},
  ...
  methods: {
    getRandomList(count) {
      const shuffledList = [...local_list].sort(() => 0.5 - Math.random()); // 随机打乱数组
      return shuffledList.slice(0, count); // 返回前 count 条数据
    },
    getContent() {
      ...
      axios.get('https://v1.jinrishici.com/all.json', axiosConfig)
        .then(response => {
          let author = ` • <span style='font-size: 13px;'>` + response.data.author + `</span>`;
          this.list.push({
            'title': response.data.origin + author,
            'content': response.data.content
          });
        })
        .catch(() => {
          // 网络请求失败时使用本地 local_list，确保不重复
          this.list = this.mergeUnique(this.list, this.getRandomList(10));
        });

      axios.get('https://v1.hitokoto.cn', axiosConfig)
        .then(response => {
          ...
          this.list.push({
            'title': response.data.from + author,
    ...
    mergeUnique(existingList, newList) {
    ...
    getShowType(type) {
      const matchedType = this.type_boxx_list.find(item => item.type === type);
      this.class_boxx = matchedType ? matchedType.class : this.type_boxx_list[0].class;
    },
    getboxx() {
      // 随机获取list中的内容
      return this.boxx = this.list[Math.floor(Math.random() * this.list.length)];
    },
    checkTitleAndConten() {
    ...
    dynamicUpdateType(time) {
      ...
      if (time !== '' && time !== 'false' && /^\d+$/.test(time)) {
        let index = this.type_boxx_list.findIndex(item => item.type === this.type);
        clearInterval(this.interval);
        this.interval = setInterval(() => {
          index = (index + 1) % this.type_boxx_list.length;
          this.getShowType(this.type_boxx_list[index].type);
          this.checkTitleAndConten();
        }, time);
      }
    }
